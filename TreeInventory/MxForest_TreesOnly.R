...
# SR values for PSUs per ecoregion (table s3) - STILL NEEDS UPDATE ----
# cyle 1
x <- FullStack_V1 |> 
  filter(Muestreado1 == 1 & Plot_S1 == 4) |> 
  group_by(DESECON2) |> 
...
###################  COMBINED TABLE OF CHANGE CALCULATIONS  ################################################
# STEP 1: left join 
Eco_Changes <- PSU_SRC |> 
  left_join(ECO_SRC,
            by = c("DESECON2", "Comparison"))
Eco_Changes |> 
  print(n=100)
