...
library(sf)         #to rasterize ecoregions shapefile
library(writexl)    #to write xlsx files
...
Sec.09 |> 
  filter(Tipo_cgl_estandar == "Inaccesible (social)") |> 
  count()
...
Sec.14 |> 
  filter(Tipo_cgl_C3 == "Inaccesible") |> 
  count()
...
FullStack_V1 <- Base |> 
  ...
  mutate(Plot_S3 = case_when(is.na(Plot_S3) ~ 0,
                             T ~ Plot_S3)) |> 
  mutate(DESECON2 = case_when(DESECON2 == "California Mediterranea" ~ "Mediterranean California", 
                              DESECON2 == "Desiertos Calidos" ~ "Warm Deserts", 
                              DESECON2 == "Piedemonte de la Sierra Madre Occidental" ~ "Western Sierra Madre Piedmont",
                              DESECON2 == "Altiplanicie Mexicana" ~ "Mexican High Plateau", 
                              DESECON2 == "Planicie semiarida de Tamaulipas-Texas" ~ "Tamaulipas-Texas Semi-Arid Plain", 
                              DESECON2 == "Planicie costera de Texas-Louisiana" ~ "Texas-Louisiana Coastal Plain",
                              DESECON2 == "Planicie y Lomerios de la Peninsula de Yucatan" ~ "Plain and Hills of the Yucatan Peninsula", 
                              DESECON2 == "Planicie Costera y Lomerios Humedos del Golfo de Mexico" ~ "Humid Gulf of Mexico Coastal Plains and Hills", 
                              DESECON2 == "Planicies y Lomerios del Occidente" ~ "Western Pacific Plain and Hills",
                              DESECON2 == "Planicie Costera y Lomerios del Soconusco" ~ "Coastal Plain and Hills of Soconusco", 
                              DESECON2 == "Sierra de Los Tuxtlas" ~ "Sierra Los Tuxtlas", 
                              DESECON2 == "Planicie Costera y Lomerios del Pacifico Sur" ~ "Southern Pacific Coastal Plain and Hills",
                              DESECON2 == "Planicie Costera, Lomerios y Canones del Occidente" ~ "Western Pacific Coastal Plain, Hills and Canyons", 
                              DESECON2 == "Depresiones Intermontanas" ~ "Interior Depressions", 
                              DESECON2 == "Planicies Costeras y Lomerios Secos del Golfo de Mexico" ~ "Dry Gulf of Mexico Coastal Plains and Hills",
                              DESECON2 == "Sierra y Planicies de El Cabo" ~ "Sierra and Plains of El Cabo", 
                              DESECON2 == "Planicie Noroccidental de la  Peninsula de Yucatan" ~ "Northwestern Plain of the Yucatan Peninsula", 
                              DESECON2 == "Sierra Madre Centroamericana y Altos de Chiapas" ~ "Central American Sierra Madre and Chiapas Highlands",
                              DESECON2 == "Sierra Madre Occidental" ~ "Western Sierra Madre", 
                              DESECON2 == "Sierra Madre Oriental" ~ "Eastern Sierra Madre", 
                              DESECON2 == "Sierra Madre del Sur" ~ "Southern Sierra Madre",
                              DESECON2 == "Sistema Neovolcanico Transversal" ~ "Transversal Neo-Volcanic System", 
                              DESECON2 == "Cuerpos de agua" ~ "Water Bodies",
                              T ~ DESECON2)) |> 
  ungroup()
...
FullStack_V1 |> 
  ...
  filter(Plot_S1 == 4) |> 
  summarise(avg = mean(SC1Total, na.rm = T),
            med = median(SC1Total, na.rm = T))

FullStack_V1 |> 
  filter(Muestreado2 == 1) |> 
  filter(Plot_S2 == 4) |> 
  summarise(avg = mean(SC2Total, na.rm = T),
            med = median(SC2Total ,na.rm = T))

FullStack_V1 |> 
  filter(Muestreado3 == 1) |> 
  filter(Plot_S3 == 4) |> 
  summarise(avg = mean(SC3Total, na.rm = T),
            med = median(SC3Total, na.rm = T))
...
# status of spec rich, J, and TE
a <- FullStack_V1 |> 
  filter(Muestreado1 == 1 & Muestreado2 == 1) |> 
  filter(Plot_S1 == 4 & Plot_S2 == 4) |> 
  summarise(meanSR = mean(SC12, na.rm = T),
            medianSR = median(SC12, na.rm = T),
            sdSR = sd(SC12, na.rm = T),
            meanJ = mean(J12, na.rm = T),
            medianJ = median(J12, na.rm = T),
            sdJ = sd(J12, na.rm = T),
            meanTE = mean(TE12, na.rm = T),
            medianTE = median(TE12, na.rm = T),
            sdTE = sd(TE12, na.rm = T))
a

b <- FullStack_V1 |> 
  filter(Muestreado2 == 1 & Muestreado3 == 1) |> 
  filter(Plot_S2 == 4 & Plot_S3 == 4) |> 
  summarise(meanSR = mean(SC23, na.rm = T),
            medianSR = median(SC23, na.rm = T),
            sdSR = sd(SC23, na.rm = T),
            meanJ = mean(J23, na.rm = T),
            medianJ = median(J23, na.rm = T),
            sdJ = sd(J23, na.rm = T),
            meanTE = mean(TE23, na.rm = T),
            medianTE = median(TE23, na.rm = T),
            sdTE = sd(TE23, na.rm = T))

c <- FullStack_V1 |> 
  filter(Muestreado1 == 1 & Muestreado3 == 1) |> 
  filter(Plot_S1 == 4 & Plot_S3 == 4) |> 
  summarise(meanSR = mean(SC13, na.rm = T),
            medianSR = median(SC13, na.rm = T),
            sdSR = sd(SC13, na.rm = T),
            meanJ = mean(J13, na.rm = T),
            medianJ = median(J13, na.rm = T),
            sdJ = sd(J13, na.rm = T),
            meanTE = mean(TE13, na.rm = T),
            medianTE = median(TE13, na.rm = T),
            sdTE = sd(TE13, na.rm = T))
...
# SR values for PSUs per ecoregion (table s4) ----
# STEP 1: assign levels ----
Lvl1 <- c("Mediterranean California")
Lvl2 <- c("Warm Deserts")
Lvl3 <- c("Western Sierra Madre Piedmont", "Mexican High Plateau")
Lvl4 <- c("Tamaulipas-Texas Semi-Arid Plain", "Texas-Louisiana Coastal Plain")
Lvl5 <- c("Plain and Hills of the Yucatan Peninsula", "Humid Gulf of Mexico Coastal Plains and Hills", 
          "Western Pacific Plain and Hills", "Coastal Plain and Hills of Soconusco", "Sierra Los Tuxtlas")
Lvl6 <- c("Southern Pacific Coastal Plain and Hills", "Western Pacific Coastal Plain, Hills and Canyons",
          "Interior Depressions", "Dry Gulf of Mexico Coastal Plains and Hills", "Sierra and Plains of El Cabo",
          "Northwestern Plain of the Yucatan Peninsula")
Lvl7 <- c("Central American Sierra Madre and Chiapas Highlands", "Western Sierra Madre", "Eastern Sierra Madre", 
          "Southern Sierra Madre", "Transversal Neo-Volcanic System")
Lvl8 <- c("Water Bodies")
...
# STEP 2: the actual thing ----
# cyle 1
SR1.eco <- FullStack_V1 |> 
  filter(Muestreado1 == 1 & Plot_S1 == 4) |> 
  group_by(DESECON2) |> 
  summarise(PSUs = n(),
            MaxSR = max(SC1, na.rm = T),
            MinSR = min(SC1, na.rm = T),
            meanSR = mean(SC1, na.rm = T),
            medianSR = median(SC1, na.rm = T),
            sdSR = sd(SC1, na.rm = T),
            MaxJ = max(J1, na.rm = T),
            MinJ = min(J1, na.rm = T),
            meanJ = mean(J1, na.rm = T),
            medianJ = median(J1, na.rm = T),
            sdJ = sd(J1, na.rm = T),
            MaxTE = max(TE1, na.rm = T),
            MinTE = min(TE1, na.rm = T),
            meanTE = mean(TE1, na.rm = T),
            medianTE = median(TE1, na.rm = T),
            sdTE = sd(TE1, na.rm = T)) |> 
  mutate(Cycle = 1)

# cyle 2
SR2.eco<- FullStack_V1 |> 
  filter(Muestreado2 == 1 & Plot_S2 == 4) |> 
  group_by(DESECON2) |> 
  summarise(PSUs = n(),
            MaxSR = max(SC2, na.rm = T),
            MinSR = min(SC2, na.rm = T),
            meanSR = mean(SC2, na.rm = T),
            medianSR = median(SC2, na.rm = T),
            sdSR = sd(SC2, na.rm = T),
            MaxJ = max(J2, na.rm = T),
            MinJ = min(J2, na.rm = T),
            meanJ = mean(J2, na.rm = T),
            medianJ = median(J2, na.rm = T),
            sdJ = sd(J2, na.rm = T),
            MaxTE = max(TE2, na.rm = T),
            MinTE = min(TE2, na.rm = T),
            meanTE = mean(TE2, na.rm = T),
            medianTE = median(TE2, na.rm = T),
            sdTE = sd(TE2, na.rm = T),) |> 
  mutate(Cycle = 2)

# cyle 3
SR3.eco <- FullStack_V1 |> 
  filter(Muestreado3 == 1 & Plot_S3 == 4) |> 
  group_by(DESECON2) |> 
  summarise(PSUs = n(),
            MaxSR = max(SC3, na.rm = T),
            MinSR = min(SC3, na.rm = T),
            meanSR = mean(SC3, na.rm = T),
            medianSR = median(SC3, na.rm = T),
            sdSR = sd(SC3, na.rm = T),
            MaxJ = max(J3, na.rm = T),
            MinJ = min(J3, na.rm = T),
            meanJ = mean(J3, na.rm = T),
            medianJ = median(J3, na.rm = T),
            sdJ = sd(J3, na.rm = T),
            MaxTE = max(TE3, na.rm = T),
            MinTE = min(TE3, na.rm = T),
            meanTE = mean(TE3, na.rm = T),
            medianTE = median(TE3, na.rm = T),
            sdTE = sd(TE3, na.rm = T),) |> 
  mutate(Cycle = 3)

PSU_SR.eco <- rbind(SR1.eco,SR2.eco,SR3.eco) |> 
  select(DESECON2, Cycle, PSUs, meanSR, medianSR, sdSR, meanJ, medianJ, sdJ, meanTE, medianTE, sdTE) |> 
  arrange(desc(DESECON2), Cycle)

PSU_SR.eco |> 
  select(Cycle, PSUs, DESECON2, meanSR, medianSR, sdSR, meanJ, medianJ, sdJ, meanTE, medianTE, sdTE) |> 
  print(n=100)

#write_xlsx(PSU_SR,  "PSU_SR.xlsx")
...
# STEP 2: the actual thing ----
# cyle 1
SRC12.eco <- FullStack_V1 |> 
  filter(Muestreado1 == 1 & Muestreado2 == 1) |> 
  filter(Plot_S1 == 4 & Plot_S2 == 4) |> 
  group_by(DESECON2) |> 
  select(SC12, J12, TE12) |> 
  summarise(PSUs_Comparison = n(),
            meanSC = mean(SC12, na.rm = T),
            medianSC = median(SC12, na.rm = T),
            sdSC = sd(SC12, na.rm = T),
            meanJC = mean(J12, na.rm = T),
            medianJC = median(J12, na.rm =T),
            sdJC = sd(J12, na.rm = T),
            meanTEC = mean(TE12, na.rm = T),
            medianTEC = median(TE12, na.rm =T),
            sdTEC = sd(TE12, na.rm = T)) |> 
  mutate(Comparison = "C12",
         Cycle = 1)

# cyle 2
SRC23.eco <- FullStack_V1 |> 
  filter(Muestreado2 == 1 & Muestreado3 == 1) |> 
  filter(Plot_S2 == 4 & Plot_S3 == 4) |> 
  group_by(DESECON2) |> 
  select(SC23, J23, TE23) |> 
  summarise(PSUs_Comparison = n(),
            meanSC = mean(SC23, na.rm = T),
            medianSC = median(SC23, na.rm = T),
            sdSC = sd(SC23, na.rm = T),
            meanJC = mean(J23, na.rm = T),
            medianJC = median(J23, na.rm =T),
            sdJC = sd(J23, na.rm = T),
            meanTEC = mean(TE23, na.rm = T),
            medianTEC = median(TE23, na.rm =T),
            sdTEC = sd(TE23, na.rm = T)) |>
  mutate(Comparison = "C23",
         Cycle = 2)

# cyle 3
SRC13.eco <- FullStack_V1 |> 
  filter(Muestreado1 == 1 & Muestreado3 == 1) |> 
  filter(Plot_S1 == 4 & Plot_S3 == 4) |> 
  group_by(DESECON2) |> 
  select(SC13, J13, TE13) |> 
  summarise(PSUs_Comparison = n(),
            meanSC = mean(SC13, na.rm = T),
            medianSC = median(SC13, na.rm = T),
            sdSC = sd(SC13, na.rm = T),
            meanJC = mean(J13, na.rm = T),
            medianJC = median(J13, na.rm =T),
            sdJC = sd(J13, na.rm = T),
            meanTEC = mean(TE13, na.rm = T),
            medianTEC = median(TE13, na.rm =T),
            sdTEC = sd(TE13, na.rm = T)) |>
  mutate(Comparison = "C13",
         Cycle = 3)

PSU_SRC.eco <- rbind(SRC12.eco,SRC23.eco,SRC13.eco) |> 
...
## ENDE ----
# Combination (imagine Drake) of both tables ----

PSU_Eco <- left_join(PSU_SR.eco, PSU_SRC.eco |> select(DESECON2, Cycle, Comparison, PSUs_Comparison, meanSC, medianSC, meanJC, medianJC, meanTEC, medianTEC),
                     by = c("DESECON2", "Cycle"))


#write_xlsx(PSU_Eco,  "PSU_Eco.xlsx")
...
result <- t.test(t.base$J23, conf.level = 0.95)
result
...
DESECON2_filter <- Comp_C_Diagnostics_V5 |> 
  left_join(Base |> select(Cluster_ID, DESECON2),
            by = "Cluster_ID")


# STEP 1: FullStack_V1 ----
FullStack_V1_mean <- Base |> 
  left_join(DESECON2_filter |> 
              select(-c("X", "Y")) |> 
              ungroup() |> 
              filter(Cycle == 1) |> 
              group_by(Cycle, DESECON2) |> 
              summarise(DBH1 = mean(AvgDbh, na.rm = T),
                        CD1 = mean(AvgCrownDiameter, na.rm = T),
                        CH1 = mean(AvgCrownHeight, na.rm = T),
                        CA1 = mean(AvgCrownArea, na.rm = T),
                        SC1 = mean(species_count, na.rm = T),
                        TH1 = mean(AvgTreeHeight, na.rm = T),
                        TE1 = mean(total_entries, na.rm = T),
                        J1 = mean(J, na.rm = T)) |> 
              ungroup() |> 
              select(DESECON2, DBH1, CD1, CH1, CA1, SC1, TH1, TE1, J1),
            by = "DESECON2") |> 
  left_join(DESECON2_filter |> 
              select(-c("X", "Y")) |> 
              ungroup() |> 
              filter(Cycle == 2) |> 
              group_by(Cycle, DESECON2) |>
              summarise(DBH2 = mean(AvgDbh, na.rm = T),
                        CD2 = mean(AvgCrownDiameter, na.rm = T),
                        CH2 = mean(AvgCrownHeight, na.rm = T),
                        CA2 = mean(AvgCrownArea, na.rm = T),
                        SC2 = mean(species_count, na.rm = T),
                        TH2 = mean(AvgTreeHeight, na.rm = T),
                        TE2 = mean(total_entries, na.rm = T),
                        J2 = mean(J, na.rm = T)) |> 
              ungroup() |> 
              select(DESECON2, DBH2, CD2, CH2, CA2, SC2, TH2, TE2, J2),
            by = "DESECON2") |> 
  left_join(DESECON2_filter |> 
              select(-c("X", "Y")) |> 
              ungroup() |> 
              filter(Cycle == 3) |> 
              group_by(Cycle, DESECON2) |>
              summarise(DBH3 = mean(AvgDbh, na.rm = T),
                        CD3 = mean(AvgCrownDiameter, na.rm = T),
                        CH3 = mean(AvgCrownHeight, na.rm = T),
                        CA3 = mean(AvgCrownArea, na.rm = T),
                        SC3 = mean(species_count, na.rm = T),
                        TH3 = mean(AvgTreeHeight, na.rm = T),
                        TE3 = mean(total_entries, na.rm = T),
                        J3 = mean(J, na.rm = T)) |> 
              ungroup() |> 
              select(DESECON2, DBH3, CD3, CH3, CA3, SC3, TH3, TE3, J3),
            by = "DESECON2") |> 
  # Exchanging NAs with Zeros (except for J)
  mutate(
    # Tree Entries
...
# STEP 2: univariate change calculations -----------------------------------
FullStack_V1_mean_2 <- FullStack_V1_mean |> 
  ungroup() |> 
  select(-c(1:9)) |> 
  distinct() |> 
  mutate(CH1 = case_when(is.na(CH1) ~ 0,
                         T ~ CH1)) |> 
  distinct() |> 
  # Calculate univariate changes in TreeCounts (TE) and SpeciesCounts (SC)
  mutate(TE12 = TE2 - TE1,
...
# OTHER STUFF - STILL NEEDS UPDATE ----
# table counting PSUs per Ecoregion for species richness
FullStack_V1 |> 
  filter(Muestreado2 == 1) |> 
  filter(Plot_S2 == 4) |> 
...
# write.csv(Cycle12, "iMAD_Data_12_TreesOnly.csv") # 25 iterations
########### CUT -------------------------------------------------------------------------------------------------
...
# write.csv(Cycle23, "iMAD_Data_23_TreesOnly.csv") # 31 iterations
########### CUT -------------------------------------------------------------------------------------------------
...
# write.csv(Cycle13, "iMAD_Data_13_TreesOnly.csv") # 32 iterations
##################################     END      ##################################################################
...
################## 1) Comparison 1-2 -------------------------------------------
#### STEP 1: Load data ----
iMAD_results_12 <- Raw.04 <- fread(here("data", "iMAD", "[1] iMAD Results", "iMAD_results_12_TreesOnly.csv"))
...
iMAD_results_12 |> 
  filter(direction_of_change != 0) |> 
  summarise(meanD = mean(direction_of_change, na.rm = T),
            medianD = median(direction_of_change, na.rm = T),
            sdD = sd(direction_of_change, na.rm = T)) 
...
################## 2) Comparison 2-3 -------------------------------------------
#### STEP 1: Load data ----
iMAD_results_23 <- Raw.04 <- fread(here("data", "iMAD", "[1] iMAD Results", "iMAD_results_23_TreesOnly.csv"))
#### STEP 2: Rename Columns + Attach Comparison column ----
...
################## 3) Comparison 1-3 -------------------------------------------
#### STEP 1: Load data ----
iMAD_results_13 <- Raw.04 <- fread(here("data", "iMAD", "[1] iMAD Results", "iMAD_results_13_TreesOnly.csv"))
#### STEP 2: Rename Columns + Attach Comparison column ----
...
## STEP 2: rejoin ecoregions to iMAD ----
iMAD_Eco <- iMAD_results |> 
  mutate(X = round(X, digits = 3),
         Y = round(Y, digits = 3)) |> 
  select(Comparison, Col_1, chi_squared, direction_of_change, X, Y) |> 
  left_join(Base |> ungroup() |> mutate(X = round(X, digits = 3), Y = round(Y, digits = 3)) |> select(X, Y, DESECON1, DESECON2)) |> 
  mutate(DESECON2 = case_when(DESECON2 == "California Mediterranea" ~ "Mediterranean California", 
                              DESECON2 == "Desiertos Calidos" ~ "Warm Deserts", 
                              DESECON2 == "Piedemonte de la Sierra Madre Occidental" ~ "Western Sierra Madre Piedmont",
                              DESECON2 == "Altiplanicie Mexicana" ~ "Mexican High Plateau", 
...
iMAD_Eco |> 
  select(DESECON2) |> 
  filter(DESECON2 == "Mexican High Plateau")
...
iMAD_Eco |> 
  ungroup() |> 
  group_by(DESECON2, Comparison) |>
  summarise(weak = sum(chi_squared == 1),
            moderate = sum(chi_squared == 2),
            strong = sum(chi_squared == 3)) |> 
  print(n = 100)

# long format 
iMAD_Eco |> 
  ungroup() |> 
  group_by(DESECON2, Comparison, chi_squared) |>
  filter(chi_squared != 0) |> 
  summarise(Sites = n()) |> 
  mutate(chi_squared = case_when(chi_squared == 1 ~ "weak",
                                 chi_squared == 2 ~ "moderate",
                                 chi_squared == 3 ~ "strong")) |> 
  print(n = 100)
...
iMAD_Eco$DESECON2 <- factor(iMAD_Eco$DESECON2, levels = all_levels)
#iMAD_Eco$DESECON2 <- factor(iMAD_Eco$DESECON2, levels = rev(levels(iMAD_Eco$DESECON2)))
...
iMAD_Eco |> 
  ...
  mutate(chi_squared = case_when(chi_squared == 1 ~ "weak",
                                 chi_squared == 2 ~ "moderate",
                                 chi_squared == 3 ~ "strong")) |> 
  rename(Intensity = chi_squared) |> 
  mutate(Intensity = factor(Intensity, levels = c("weak", "moderate", "strong"))) |> 
  ggplot(aes(x = Intensity, y = Sites, fill = Comparison)) +
  geom_bar(stat = "identity") +
  theme_bw() +
  xlab("Change Intensity") + ylab("# of Samling Sites") +
  facet_wrap(~DESECON2)

## STEP 3: directonality plots ----
# STEP 1: assign levels ----
Lvl1 <- c("Mediterranean California")
Lvl2 <- c("Warm Deserts")
Lvl3 <- c("Western Sierra Madre Piedmont", "Mexican High Plateau")
...
iMAD_Eco |> 
  ungroup() |> 
  group_by(DESECON2, Comparison, direction_of_change) |>
  filter(chi_squared != 0) |> 
  filter(!is.na(DESECON2)) |> 
  summarise(Sites = n()) |> 
  mutate(direction_of_change = case_when(direction_of_change == 1 ~ "WI",
                                         direction_of_change == 2 ~ "MI",
                                         direction_of_change == 3 ~ "SI",
                                         direction_of_change == -1 ~ "WD",
                                         direction_of_change == -2 ~ "MD",
                                         direction_of_change == -3 ~ "SD"),
         Comparison = case_when(Comparison == "Cycle12" ~ "Comparison 1-2",
                                Comparison == "Cycle23" ~ "Comparison 2-3",
                                Comparison == "Cycle13" ~ "Comparison 1-3")) |> 
  mutate(direction_of_change = factor(direction_of_change, levels = c("SD", "MD", "WD", "WI", "MI", "SI"))) |> 
  mutate(Comparison = factor(Comparison, levels = c("Comparison 1-2", "Comparison 2-3", "Comparison 1-3"))) |> 
  ggplot(aes(x = direction_of_change, y = Sites, fill = Comparison)) +
  geom_bar(stat = "identity", position = "dodge", show.legend = F) +
  scale_fill_manual(values=c(`Comparison 1-2`="black", `Comparison 2-3`="#666666", `Comparison 1-3`="#aaaaaa")) +
  theme_bw() +
  theme(strip.text.x = element_text(size = 7)) +
  xlab("Identified Change") + ylab("Conglomerates (#)") +
  facet_wrap(~DESECON2) 
...
FullStack_V1 %>% 
  ...
  rename(`Comparison 1-2` = SC12, `Comparison 2-3`= SC23, `Comparison 1-3`= SC13) |> 
  pivot_longer(c(`Comparison 1-2`, `Comparison 2-3`, `Comparison 1-3`), names_to = "Comparison", values_to = "Species_Richness") |> 
  mutate(Comparison = factor(Comparison, levels = c("Comparison 1-2", "Comparison 2-3", "Comparison 1-3"))) |> 
  ...
  ylab("Changes in Species Richness (ΔS)") +
  xlab("") 
...
FullStack_V1 %>% 
  mutate(J12 = case_when(Muestreado1 != 1 | Muestreado2 != 1 | Plot_S1 != 4 | Plot_S2 != 4 ~ NA,
         ...
         J13 = case_when(Muestreado1 != 1 | Muestreado3 != 1 | Plot_S1 != 4 | Plot_S3 != 4 ~ NA,
                          T ~ J13)) |> 
  select(J12, J23, J13) |> 
  rename(`Comparison 1-2` = J12, `Comparison 2-3`= J23, `Comparison 1-3`= J13) |>
  pivot_longer(c(`Comparison 1-2`, `Comparison 2-3`, `Comparison 1-3`), names_to = "Comparison", values_to = "Evenness") |> 
  mutate(Comparison = factor(Comparison, levels = c("Comparison 1-2", "Comparison 2-3", "Comparison 1-3"))) |> 
  ggplot( aes(x= Comparison, y= Evenness, fill=Comparison)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") + 
  ...
  ylab("Changes in Pielou's Evenness (ΔJ)") +
  xlab("") 
...
FullStack_V1 %>% 
  ...
  rename(`Comparison 1-2` = TE12, `Comparison 2-3`= TE23, `Comparison 1-3`= TE13) |>
  pivot_longer(c(`Comparison 1-2`, `Comparison 2-3`, `Comparison 1-3`), names_to = "Comparison", values_to = "Tree_Abundance") |> 
  mutate(Comparison = factor(Comparison, levels = c("Comparison 1-2", "Comparison 2-3", "Comparison 1-3"))) |> 
  ggplot( aes(x= Comparison, y= Tree_Abundance, fill=Comparison)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") + 
  geom_violin(scale = "area", adjust = 1, width = 1) +
  scale_fill_manual(values=c(`Comparison 1-2`="black", `Comparison 2-3`="#666666", `Comparison 1-3`="#aaaaaa")) +
  theme_bw() +
  ...
  ylab("Changes in Tree Abundance (Δind)") +
  xlab("") 
...
FullStack_V1 |> 
  ...
  xlab("Species Richness Cycle 1 (# of species)") + ylab("Species Richness Cycle 2 (# of species)") +
  theme_light() +
...
FullStack_V1 |> 
  ...
  coord_cartesian(xlim = c(0, 60), ylim = c(0,60)) +
  geom_abline(color = "red") +
  xlab("Species Richness Cycle 2 (# of species)") + ylab("Species Richness Cycle 3 (# of species)") +
  theme_light() +
  theme(axis.title.x = element_text(family = "ARL", size = 11),
        axis.title.y = element_text(family = "ARL", size = 11)) +
...
FullStack_V1 |> 
  ...
  coord_cartesian(xlim = c(0, 60), ylim = c(0,60)) +
  geom_abline(color = "red") +
  xlab("Species Richness Cycle 1 (# of species)") + ylab("Species Richness Cycle 3 (# of species)") +
  theme_light() +
  theme(axis.title.x = element_text(family = "ARL", size = 11),
        axis.title.y = element_text(family = "ARL", size = 11)) +
...
FullStack_V1 |> 
  ...
  geom_abline(color = "red") +
  xlab("Pielou`s Evenness Cycle 1 (J)") + ylab("Pielou`s Evenness Cycle 2 (J)") +
  theme_light() +
  theme(axis.title.x = element_text(family = "ARL", size = 11),
        axis.title.y = element_text(family = "ARL", size = 11)) +
...
FullStack_V1 |> 
  ...
  coord_cartesian(xlim = c(0, 1), ylim = c(0, 1)) +
  geom_abline(color = "red") +
  xlab("Pielou`s Evenness Cycle 2 (J)") + ylab("Pielou`s Evenness Cycle 3 (J)") +
  theme_light() +
  theme(axis.title.x = element_text(family = "ARL", size = 11),
        axis.title.y = element_text(family = "ARL", size = 11)) +
...
FullStack_V1 |> 
  ...
  coord_cartesian(xlim = c(0, 1), ylim = c(0, 1)) +
  geom_abline(color = "red") +
  xlab("Pielou`s Evenness Cycle 1 (J)") + ylab("Pielou`s Evenness Cycle 3 (J)") +
  theme_light() +
  theme(axis.title.x = element_text(family = "ARL", size = 11),
        axis.title.y = element_text(family = "ARL", size = 11)) +
...
FullStack_V1 |> 
  ...
  geom_abline(color = "red") +
  xlab("Tree Abundance Cycle 1 (# of individuals)") + ylab("Tree Abundance Cycle 2 (# of individuals)") +
  theme_light() +
  theme(axis.title.x = element_text(family = "ARL", size = 11),
        axis.title.y = element_text(family = "ARL", size = 11)) +
...
FullStack_V1 |> 
  ...
  coord_cartesian(xlim = c(0, 400), ylim = c(0, 400)) +
  geom_abline(color = "red") +
  xlab("Tree Abundance Cycle 2 (# of individuals)") + ylab("Tree Abundance Cycle 3 (# of individuals)") +
  theme_light() +
  theme(axis.title.x = element_text(family = "ARL", size = 11),
        axis.title.y = element_text(family = "ARL", size = 11)) +
...
FullStack_V1 |> 
  ...
  coord_cartesian(xlim = c(0, 400), ylim = c(0, 400)) +
  geom_abline(color = "red") +
  xlab("Tree Abundance Cycle 1 (# of individuals)") + ylab("Tree Abundance Cycle 3 (# of individuals)") +
  theme_light() +
  theme(axis.title.x = element_text(family = "ARL", size = 11),
        axis.title.y = element_text(family = "ARL", size = 11)) +
...
FullStack_V1 |> 
  filter(Muestreado1 == 1 & Muestreado2 == 1 & Plot_S1 == 4 & Plot_S2 == 4) |> 
  ggplot(aes(x = TE12, y = SC12)) +
  geom_point(position = "jitter", color = "black") +
  coord_cartesian(xlim = c(-300, 300), ylim = c(-50, 50)) +
  geom_abline(color = "red") +
  xlab("Tree Abundance Change (Comparison 1-2)") + ylab("Species Richness Change (Comparison 1-2)") +
  theme_light() +
  theme(axis.title.x = element_text(family = "ARL", size = 11),
        axis.title.y = element_text(family = "ARL", size = 11)) +
  annotate("text", x=20, y=400, label = "n = 17 239", size = 4, hjust = 0.5, vjust = 0.5)
...
FullStack_V1 |> 
  filter(Muestreado1 == 1 & Muestreado2 == 1 & Plot_S1 == 4 & Plot_S2 == 4) |> 
  ggplot(aes(x = TE12, y = J12)) +
  geom_point(position = "jitter", color = "black") +
  coord_cartesian(xlim = c(-300, 300), ylim = c(-1, 1)) +
  geom_abline(color = "red") +
  xlab("Tree Abundance Change (Comparison 1-2)") + ylab("Evenness Change (Comparison 1-2)") +
  theme_light() +
  theme(axis.title.x = element_text(family = "ARL", size = 11),
        axis.title.y = element_text(family = "ARL", size = 11)) +
  annotate("text", x=20, y=400, label = "n = 17 239", size = 4, hjust = 0.5, vjust = 0.5)
...
Lvl1 <- c("Mediterranean California")
Lvl2 <- c("Warm Deserts")
Lvl3 <- c("Western Sierra Madre Piedmont", "Mexxican High Plateau")
Lvl4 <- c("Tamaulipas-Texas Semi-Arid Plain", "Texas-Louisiana Coastal Plain")
Lvl5 <- c("Plain and Hills of the Yucatan Peninsula", "Humid Gulf of Mexico Coastal Plains and Hills", 
          "Western Pacific Plain and Hills", "Coastal Plain and Hills of Soconusco", "Sierra Los Tuxtlas")
Lvl6 <- c("Southern Pacific Coastal Plain and Hills", "Western Pacific Coastal Plain, Hills and Canyons",
...
colors_for_values <- c("Mediterranean California" = "#ff6554", 
                       "Warm Deserts" = "#ffa114", 
                       "Western Sierra Madre Piedmont" = "#fceb02",
                       "Mexxican High Plateau" = "#fceb02", 
                       "Tamaulipas-Texas Semi-Arid Plain" = "#89c7d1", 
                       "Texas-Louisiana Coastal Plain" = "#89c7d1",
                       "Plain and Hills of the Yucatan Peninsula" = "#8ecc9e", 
                       "Humid Gulf of Mexico Coastal Plains and Hills" = "#8ecc9e", 
                       "Western Pacific Plain and Hills" = "#8ecc9e",
                       "Coastal Plain and Hills of Soconusco" = "#8ecc9e", 
                       "Sierra Los Tuxtlas" = "#8ecc9e", 
                       "Southern Pacific Coastal Plain and Hills" = "#7bb4ed",
                       "Western Pacific Coastal Plain, Hills and Canyons" = "#7bb4ed", 
                       "Interior Depressions" = "#7bb4ed", 
                       "Dry Gulf of Mexico Coastal Plains and Hills" = "#7bb4ed",
                       "Sierra and Plains of El Cabo" = "#7bb4ed", 
                       "Northwestern Plain of the Yucatan Peninsula" = "#7bb4ed", 
                       "Central American Sierra Madre and Chiapas Highlands" = "#b4aade",
                       "Western Sierra Madre" = "#b4aade", 
                       "Eastern Sierra Madre" = "#b4aade", 
                       "Southern Sierra Madre" = "#b4aade",
                       "Transversal Neo-Volcanic System" = "#b4aade", 
                       "Water Bodies" = "#e9ccdb")
...
FullStack_V1 %>% 
  ...
  mutate(Comparison = case_when(Comparison == "SC12" ~ "C12",
                           ...
                           T ~ Comparison)) |> 
  mutate(Comparison = factor(Comparison, levels = c("C12", "C23", "C13"))) |> 
  filter(!is.na(DESECON2) & DESECON2 != "Water Bodies") |> 
  ...
  theme_minimal() +
  ylab("") +
  xlab("Changes in Species Richness per Conglomerate (ΔS)") +
  theme(axis.title.x = element_text(family = "ARL", size = 11),
        axis.title.y = element_text(family = "ARL", size = 11))
...
FullStack_V1 %>% 
  ...
  theme_minimal() +
  ylab("") +
  xlab("Changes in Pielou's Evenness per Conglomerate (ΔJ)") +
  theme(axis.title.x = element_text(family = "ARL", size = 11),
        axis.title.y = element_text(family = "ARL", size = 11))
...
FullStack_V1 %>% 
  ...
  theme_minimal() +
  ylab("") +
  xlab("Changes in Tree Abundance (Δind)") +
  theme(axis.title.x = element_text(family = "ARL", size = 11),
        axis.title.y = element_text(family = "ARL", size = 11))
...
########### Mean and SD, Median Values  ###################
#### 1) Total Species Richness ----
FullStack_V1 |> 
  filter(Muestreado1 == 1) |> 
  filter(Plot_S1 == 4) |> 
  summarise(meanSR = mean(SC1, na.rm = T),
            sdSR = sd(SC1, na.rm = T),
            medianSC1 = median(SC1, na.rm = T),
            meanJ = mean(J1, na.rm = T),
            sdJ = sd(J1, na.rm = T),
            medianJ = median(J1, na.rm = T),
            meanTE = mean(TE1, na.rm = T),
...
FullStack_V1 |> 
  filter(Muestreado2 == 1) |> 
  filter(Plot_S2 == 4) |> 
  summarise(meanSR = mean(SC2, na.rm = T),
            sdSR = sd(SC2, na.rm = T),
            medianSR = median(SC2, na.rm = T),
            meanJ = mean(J2, na.rm = T),
            sdJ = sd(J2, na.rm = T),
...
FullStack_V1 |> 
  filter(Muestreado3 == 1) |> 
  filter(Plot_S3 == 4) |> 
  summarise(meanSR = mean(SC3, na.rm = T),
            sdSR = sd(SC3, na.rm = T),
            medianSR = median(SC3, na.rm = T),
            meanJ = mean(J3, na.rm = T),
            sdJ = sd(J3, na.rm = T),
